...
void TencentBot::init() {
    ...
    // --- 2. 通过 HV 获取每个游戏进程的 CR3 和 DLL 基地址 ---
    wchar_t dllName[] = L"mhmain.dll";
    for (size_t i = 0; i < gameMemory.processIds.size(); ++i) {
        uint64_t cr3     = hv::query_process_cr3(gameMemory.processIds[i]);
        uint64_t dllBase = hv::get_va_of_dllbase(gameMemory.processIds[i], dllName, 20);
        gameMemory.cr3Values.push_back(cr3);
        gameMemory.dllBaseAddrs.push_back(dllBase);
        BOT_LOG("TencentBot", "进程[" << i << "] CR3=0x" << std::hex << cr3
                << " DllBase=0x" << dllBase << std::dec);
    }
