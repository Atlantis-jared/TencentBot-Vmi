//
// Created by 55296 on 2026/1/19.
//

#ifndef UNTITLED_HV_H
#define UNTITLED_HV_H
#pragma once
...
namespace hv {
    ...
    **/
    ...
    inline uint64_t get_va_of_dllbase(uint64_t pid, wchar_t* dllname, uint64_t size)
    {
        hv::hypercall_input input;
        input.code = hv::hypercall_query_va_of_dllname;
        input.key = hv::hypercall_key;
        input.args[0] = (uint64_t)dllname;
        input.args[1] = size;
        input.args[2] = pid;
        return hv::vmx_vmcall(input);
    }
